...
static inline bool LatestVersion(int *major, int *minor, int *current_line) {
	...
	PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, Downloads[DOWNLOAD_VERSION].text);
	UpdateScreen();
	line++;
	if(!http_request(Downloads[DOWNLOAD_VERSION].url, Downloads[DOWNLOAD_VERSION].max_size)) {
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Failed to retrieve version");
		UpdateScreen();
		*current_line = line;
		return false;
	}
	...
	if ((*major <= NIN_MAJOR_VERSION) && (*minor <= NIN_MINOR_VERSION)) {
		...
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "You already have the latest version");
		line++;
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Download anyway? (A: Yes, B: No)");
		line++;
		UpdateScreen();
		while(true) {
			DrawBuffer();
...
static s32 Download(DOWNLOADS download_number)  {
	...
	PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, Downloads[download_number].text);
	UpdateScreen();
	...
	PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Network Initialized");
	UpdateScreen();
	...
	if (download_number == DOWNLOAD_NINTENDONT) {
		...
		if (!ret) {
			PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Download Cancelled");
			UpdateScreen();
			if (outbuf != NULL) free(outbuf);
			net_deinit();
			sleep(4);
		...
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Downloading Nintendont v%i.%i", major, minor);
		UpdateScreen();
		line++;
	}
	int i;
	...
	PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Download Complete");
	UpdateScreen();
	line++;
	...
	if(!file)
	{
		...
		if (ret == 1) {
			PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Update Complete");
			UpdateScreen();
			line++;
		}
	}
	...
	if (ret != 1)
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR_ERROR, MENU_POS_X, MENU_POS_Y + 20*line, "Update Error: %i", ret);
	else
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*line, "Restart Nintendont to complete update");
	UpdateScreen();
	if (outbuf != NULL) free(outbuf);
	net_deinit();
	sleep(4);
...
void UpdateNintendont(void) {
	...
	while(true) {
		ClearScreen();
		PrintInfo();
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X + 50, MENU_POS_Y + 20*5, "Download Nintendont");
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X + 50, MENU_POS_Y + 20*6, "Download titles.txt");
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X + 50, MENU_POS_Y + 20*7, "Download controllers.zip");
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X + 50, MENU_POS_Y + 20*8, "Return to Settings");
		PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X + 35, MENU_POS_Y + 20*(5+selected), ARROW_RIGHT);
		GRRLIB_Render();
		FPAD_Update();
