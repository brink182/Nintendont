...
//	u64 timeout = 0;
	...
	*(vu32*)0xD3003420 = 0; //make sure kernel doesnt reload
	...
	PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X + + 430, MENU_POS_Y + 20*1, "A   : Select");
	GRRLIB_Render();
	ClearScreen();//*/
		...
		|| memcmp(&(ncfg->GameID), "GCOP", 4) == 0 || memcmp(&(ncfg->GameID), "RGCO", 4) == 0)
	{
		...
		while(1)
		{
			...
			if( redraw )
			{
				PrintInfo();
				for( i=0; i < j; ++i )
					PrintFormat(DEFAULT_SIZE, TEXTCOLOR, MENU_POS_X, MENU_POS_Y + 20*4 + i * 20, "%50.50s [%.6s]%s", gi[i].Name, gi[i].ID, i == PosX ? ARROW_LEFT : " " );
				GRRLIB_Render();
				Screenshot();
				ClearScreen();
				redraw = false;
